...
export async function scanExtraFile(config: ConfigType): Promise<ConfigType> {
  ...
  if (snpFileMapConfig.snpFileMap) {
    for (const SNPKeySuffix of config.availableSNPKeySuffix) {
      ...
      for (const [realFilePath, snpPathFileSet] of Object.entries(snpFileMapConfig.snpFileMap || {}) as [
        string,
